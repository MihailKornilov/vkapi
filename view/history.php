...
function _history_insert($v=array()) {//внесение истории действий
	...
	if(!$client_id && $zayav_id) {//если указана за€вка, но не указан клиент, обновл€етс€ id клиента
...
function _history_spisok($v=array()) {
	...
	define('HIST_LOCAL', $filter['client_id'] || $filter['zayav_id'] || $filter['schet_id']); //истори€ конкретных объектов
	$spisok = $filter['js'];
...
//	   " AND `type_id` IN (39)";//todo удалить
	...
	$add = HIST_LOCAL ? '' : '<div id="history-add" class="img_add m30'._tooltip('ƒобавить событие', -60).'</div>';
	...
	if(!$all = query_value($sql))
		return array(
			...
			'spisok' =>
				$spisok.
				(PAGE1 ? '<div class="result norm">»стории по указанным услови€м нет'.$add.'</div>' : '').
				'<div class="_empty mar8">»стории по указанным услови€м нет</div>',
			'filter' => $filter
		);
	...
	$send = array(
		'all' => $all,
		'spisok' =>
			(PAGE1 ? '<div class="result norm">ѕоказан'._end($all, 'а ', 'о ')._sumSpace($all).' запис'._end($all, 'ь', 'и', 'ей').$add.'</div>' : '').
			$spisok,
		'filter' => $filter
	);
	...
	foreach($history as $r) {
		...
		$txt .= '<li'.(SA ? ' class="light"' : '').'>'.
					(SA ? '<h4 val="'.$r['id'].'">'.($r['type_id_old'] ? $r['type_id_old'].'-' : '').$r['type_id'].'</h4>' : '').//todo удалить type_id_old после перенесени€
...
function _history_types($history) {//перевод type_id в текст
	...
	foreach($history as $id => $r) {
		$txt = $types[$r['type_id']];
		foreach($str as $v) {
			// проверка услови€: ?{v1}, если v1 пуста€, то не выводитс€ строка, заключЄнна€ между пробелами с этой переменной.
			// например: <em>?{v1}</em>
			...
			//статус за€вки: название статуса, подсвеченное соответствующим цветом (дл€ mobile)
			if(strpos($txt, '^'.$v.'^') !== false) {
				$status = '';
				if($r[$v])
...
function _historyChange($name, $old, $new, $v1='', $v2='') {//возвращаетс€ элемент таблицы, если было изменение при редактировании данных
	...
	$name = $name ? '<th>'.$name.':' : '';
	return '<tr>'.$name.'<td>'.$old.'<td>ї<td>'.$new;
}

function _history_right($v=array()) {//вывод условий поиска дл€ истории действий
	$v = array(
		'client_id' => _num(@$v['client_id'])
	);
