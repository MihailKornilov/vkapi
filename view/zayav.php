...
function _zayavPoleUseInfoConst($js=false) {//формирование констант для полей заявки
	$sql = "SELECT
				*,
				0 `use_info`
			FROM `_zayav_setup`
...
function _zayav_spisok($v) {
	...
	if($filter['find']) {
		...
		if($filter['executer_id'])
			$cond .= " AND `executer_id`=".$filter['executer_id'];
	...
	$sql = "SELECT COUNT(*) FROM `_zayav` WHERE ".$cond;
