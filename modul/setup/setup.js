...
//					$('#bank-spisok').html(res.html);
	...
	setupZayavExpense = function(o) {
		...
		$('#expense_dub_check').vkHint({
			...
			left:-103,
			width:320,
			msg:'При внесении <u>расхода по заявке</u> дополнительно будет появляться галочка, ' +
				'при установке которой автоматически будет вноситься <u>расход организации</u> на ту же сумму.' +
				'<br />' +
				'<br />' +
				'Смотрите <a href="https://youtu.be/5yzQXCKWS7E" class="b" target="_blank">подробный ролик на Ютубе</a> на тему.',
			delayShow:1000
		});
...
$.fn._templateSelect = function() {//настройка для выбора шаблона для печати
	var t = $(this),
		attr_id = t.attr('id'),
		num = 1;
	...
	var html =
		'<div class="mt10">' +
			'<button class="vk small grey">Выбрать существующий шаблон</button>' +
			' или ' +
			'<button class="vk small grey" onclick="setupTemplateEdit()">Создать новый</button>' +
		'</div>',
		INP = $('#' + attr_id);

	INP.after(html);

	var DIV_BUT = INP.next(),//расположение кнопок добавления
		BUT = DIV_BUT.find('.vk:first'),//кнопка добавления нового поля
	...
	function add(template_id) {
		...
		var iconDn = template_id ? '' : ' dn',
			...
			'<div class="mb5">' +
				...
				'<div val="' + num + '" class="icon icon-setup ' + iconDn + 'icon' + num + _tooltip('Настроить шаблон', -57) + '</div>' +
				'<div class="icon icon-del icon' + num + iconDn + _tooltip('Отвязать шаблон', -53) + '</div>' +
			'</div>';
	...
	function valCreate() {//формирование значения для отправки
