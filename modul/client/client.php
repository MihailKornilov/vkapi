...
function _clientCase() {// 1 вывод информации с клиентами для приложения
...
//					  '<a href="'.URL.'&p=75&d=template&template_id=client-spisok" class="img_xls'._tooltip('Экспорт в Excel', -12, 'l').'</a>'.
					  $filter['clear'];
	$send['filter'] = $filter;
...
function _clientVal($client_id, $i=0) {//получение данных из базы об одном клиенте
	...
	if(!defined($prefix.'LOADED')) {
		...
		define($prefix.'WORKER',
			$c['worker_id'] ?
				'<a href="'.URL.'&p=65&id='.$c['worker_id'].'" class="'._tooltip('Перейти на страницу з/п сотрудника', -70).
...
($accrual ? '<td class="ci-acc w50 center curD wsnw fs12'._tooltip('Общая сумма начислений', -55)._sumSpace($accrual) : '').
 ...
 ($refund ? '<td class="ci-refund w50 center curD wsnw fs12'._tooltip('Общая сумма возвратов', -53)._sumSpace($refund) : '').
			...
			'<td class="w100">'.
				'<div class="mt5 mr5 r">'.
					($r['worker_id'] ? '<a href="'.URL.'&p=65&id='.$r['worker_id'].'" class="icon icon-worker center'._tooltip('Клиент является сотрудником<br />Перейти на страницу з/п сотрудника', -109, '', 1).'</a>' : '').
