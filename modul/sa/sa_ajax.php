...
//		xcache_unset(CACHE_PREFIX.'service');
	...
	case 'sa_tovar_cat_copy':
		if(!$id = _num($_POST['id']))
			jsonError('Некорректный id категории товара');
		...
		if($cat['app_id'] == APP_ID)
			jsonError('Категорию можно скопировать только из другого приложения');

		if(!$cat['parent_id'])
			jsonError('Копировать можно только дочернюю категорию');
		...
		if(!$parent = query_assoc($sql))
			jsonError('Родительской категории не существует');
		...
		if(!$arr = query_arr($sql))
			jsonError('Нет товаров для переноса');

		foreach($arr as $r) {
			//если товар уже был внесён, то пропуск
			$sql = "SELECT COUNT(*)
					FROM `_tovar_bind`
					WHERE `app_id`=".APP_ID."
