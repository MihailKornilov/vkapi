...
//			if(VIEWER_ADMIN)
	...
	case 'expense_add':
		...
		if(!expense_dtime_old_update_for_kupez($invoice_id, $insert_id, $sum))//todo баланс обновляется, если сегодняшняя дата
			_balans(array(
				'action_id' => 6,
				'invoice_id' => $invoice_id,
				'sum' => $sum,
...
function schet_pay_content_get($return_sum=0) {//получение содержания счёта на оплату при создании или редактировании
