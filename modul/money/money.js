var _accrualAdd = function(o) {
 ...
 (client_name ? '<tr><td class="label r">Клиент:<td>' + client_name : '') +
	...
	_expenseTab = function(dialog, o) {//таблица для внесения или редактирования расхода
		...
		var html =
			'<table class="bs10">' +
				'<tr' + (APP_ID == 3495523 || APP_ID == 3978722 ? '' : ' class="dn"') + '><td class="label r">День внесения:<td><input type="hidden" id="dtime_add" value="' + o.dtime_add + '" />' + //todo временно
	...
	.on('click', '#money-invoice .icon-setup', function() {
		var t = $(this),
			...
			html = '<div id="invoice-setup-tab">' +
						'<table class="bs10">' +
							'<tr><td class="label r">Расчётный счёт:<td><b>' + name + '</b>' +
							'<tr><td class="label r">Текущий баланс:<td>' + (balans ? '<b>' + balans + '</b> руб.' : 'не установлен') +
						'</table>' +
						'<div class="unit bg-gr1 bor-e8 over1 mar10 pad10 curP" val="1">' +
							'<b>Перевод между счетами</b>' +
							'<div class="grey pad2-7">Перевести денежные средства на другой расчётный счёт.</div>' +
						'</div>' +
				(VIEWER_ADMIN || RULE_SETUP_INVOICE ?
					(balans ?
						'<div class="unit bg-gr1 bor-e8 over1 mar10 pad10 curP" val="2">' +
							'<b>Внести деньги на счёт</b>' +
							'<div class="grey pad2-7">Внести произвольную сумму на расчётный счёт. Данная сумма не будет являться платежом.</div>' +
						'</div>' +
						'<div class="unit bg-gr1 bor-e8 over1 mar10 pad10 curP" val="7">' +
							'<b>Вывести деньги со счёта</b>' +
							'<div class="grey pad2-7">Произвести вывод денежных средств из организации.</div>' +
						'</div>'
					: '') +
						'<div class="unit bg-gr1 bor-e8 over1 mar10 pad10 curP" val="3">' +
							'<b>Установить текущую сумму</b>' +
							'<div class="grey pad2-7">Установить сумму, которая соответствует фактической сумме на расчётном счёте или наличию денег в кассе.</div>' +
						'</div>' +
					(balans ?
						'<div class="unit bg-gr1 bor-e8 over1 mar10 pad10 curP" val="4">' +
							'<b>Сброс суммы</b>' +
							'<div class="grey pad2-7">' +
								'Текущая сумма на счёте будет сброшена, но все операции по счёту, история действий будут доступны. ' +
								'Нужно если вы не хотите контролировать данный расчётный счёт.' +
							'</div>' +
						'</div>'
					: '') +
						'<div class="unit bg-gr1 bor-e8 over1 mar10 pad10 curP" val="5">' +
							'<b>Редактировать счёт</b>' +
							'<div class="grey pad2-7">Изменить название счёта и его описание. Настроить видимость для сотрудников и права.</div>' +
						'</div>' +
						'<div class="unit bg-gr1 bor-e8 over1 mar10 pad10 curP" val="6">' +
							'<b>Закрыть счёт</b>' +
							'<div class="grey pad2-7">При закрытии счёта остаток будет переведён на другой расчётный счёт.</div>' +
						'</div>'
				: '') +
					'</div>',
			dialog = _dialog({
		...
		$('#invoice-setup-tab .unit').click(function() {
			dialog.close();
