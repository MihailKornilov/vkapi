...
function _zayavStatus($id=false, $i='name') {//кеширование статусов заявки
				...
				'<div id="status-tab">'.
					'<table class="bg-fff curP">'.
						'<tr val="0" class="over1">'.
							'<td class="td-check pad8'._tooltip('Выбрать все', -34).
								_check('st0', '').
							'<td class="pad8">'.
							'<td class="r"><div class="icon icon-del mr5'._tooltip('Скрыть статусы', -90, 'r').'</div>'.
						$filter.
					'</table>'.
				'</div>'.
			'</div>';
	...
	//id статусов, которые не скрываются из общего списка
	if($id == 'show_ids') {
		$ids = array();
		foreach($arr as $r)
			if(!$r['hide'])
...
function _zayav_spisok($v) {
	...
	if($filter['f60']) {//только выбранные заявки
		$sql = "SELECT `zayav_id`
				FROM `_unit_check`
				WHERE `app_id`=".APP_ID."
				  AND `zayav_id`
...
function _zayavUnitCheckButton($v) {//отображение кнопки выбранных единиц заявок
	$sql = "SELECT COUNT(*)
			FROM `_unit_check`
			WHERE `app_id`=".APP_ID."
			  AND `zayav_id`
	...
	'</button>'.
	_check('f60', 'Выбранные заявки', $v['f60'], 1);
}
...
function _zayavDogovorPrint($v) {
	...
	$dopl = $v['sum'] - $v['avans'];
	$v['dopl'] = $dopl < 0 ? 0 : $dopl;
	$v['adres'] = $v['pasp_adres'] ? $v['pasp_adres'] : $v['adres'];
...
*/
	...
	$doc = _zayavDogovorPrintContent1($doc, $v);
	$doc = _zayavDogovorPrintContent2($doc, $v);
...
function _zayavDogovorPrintContent1($doc, $v) {//Договор оказания услуг
	if(!TEMPLATE_2)
		return $doc;
	...
	'<div class="head-name">'.
		...
		'№'.$v['nomer'].
	'</div>'.
	'<table class="city_data"><tr><td>г. Няндома<th>'._dataDog($v['data_create']).'</table>'.
	'<div class="paragraph">'.
		'<p>Общество с ограниченной ответственностью «Территория Комфорта», '.
			'в лице менеджера по продажам, '._viewer(VIEWER_ID, 'viewer_name_full').', '.
			'действующей на основании доверенности №7 от 05.04.2017 г. '.
			'именуемое в дальнейшем Исполнитель с одной стороны, и '.
			$v['fio'].($v['adres'] ? ', '.$v['adres'] : '').', именуемый(ая) в дальнейшем «Заказчик», с другой стороны, '.
			'совместно именуемые Стороны, заключили настоящий Договор возмездного оказания услуг, в дальнейшем Договор, о нижеследующем:'.
	'</div>'.

	'<div class="p-head">1. Предмет договора</div>'.
	'<div class="paragraph">'.
		'<p>1.1. В соответствии и на условиях настоящего Договора Исполнитель обязуется своими силами или силами третьих лиц оказать Заказчику услуги по изготовлению, доставке и установке (монтажу) конструкций из ПВХ (оконных блоков), дверных блоков, роллетных систем, гаражных и промышленных ворот (далее – услуга), а Заказчик обязуется создать Исполнителю необходимые условия для выполнения услуг, принять их результаты и оплатить эти услуги согласно условиям, указанным в настоящем Договоре.'.
		'<p>1.2. Полный перечень и характеристика услуг содержится в Приложении №1 к Договору, которое является неотъемлемой частью настоящего Договора.'.
		'<p>1.3. Право собственности на Изделия устанавливаемые в ходе оказания услуги, а также риск случайной гибели или случайного повреждения Изделия переходит к Заказчику после подписания «Акта сдачи-приемки»'.
		'<p>1.4. Адрес выполнения работ: '.($v['adres'] ? ', '.$v['adres'] : '___________________').
	'</div>'.

	'<div class="p-head">2. Обязанности сторон</div>'.
	'<div class="paragraph">'.
		'<p>2.1. Исполнитель обязуется выполнить услуги с соблюдением условий настоящего Договора и требований, предъявляемых к продукции данного типа и указанных в ГОСТах №23166-99 «Блоки оконные ТУ», №30970-2002 «Блоки оконные из ПВХ» для оконных блоков, в рабочей документации разработчиков систем профилей для дверных блоков, в «Инструкции по изготовлению роллет», «Инструкции по изготовлению ворот», в ГОСТах №111-2001 «Стекло листовое», №24866-99 «Стеклопакеты клееные строительного назначения».'.
		'<p>2.2. Предварительный согласованный срок выполнения услуг,предусмотренных настоящим Договором, составляет 20 (двадцать) рабочих дней. Окончательный срок выполнения не более 30 (тридцати) рабочих дней с момента поступления от Заказчика оплаты согласно условий пункта 3. настоящего Договора и обеспечения Заказчиком условий пунктов 2.3. и 2.4. Данные сроки предусмотрены по стандартным изделиям. В случае заказа сложных и цветных изделий, срок Договора увеличивается на количество дополнительных дней на изготовление сложной конструкции, и указывается в Приложении №1.'.
		'<p>2.3. Заказчик обязуется обеспечить доступ монтажников и подвод электропитания к оконным проемам, защиту личного имущества, напольных покрытий от пыли и повреждений. Исполнитель не отвечает за сохранность стеновых покрытий в зоне работ. Исполнитель не несёт ответственности за нарушение элементов конструкций фасадов зданий при выполнении монтажных и отделочных работ, возникших вследствие ветхости строений и наличия скрытых строительных дефектов. Восстановительные работы проводятся по желанию и за счёт Заказчика. В стоимость отделки откосов не входит герметизация наружного шва до 4 см шириной.'.
		'<p>2.4. Заказчик обязуется принять меры по обеспечению отсутствия автотранспорта на тротуаре под оконными проемами. В случае, если Заказчик не принял данные меры и монтаж/демонтаж осуществить невозможно, Заказчик оплачивает дополнительный выезд монтажной бригады из расчета 1000 (одна тысяча) руб./выезд, при этом окончательный срок выполнения договора составит 10 (десять) рабочих дней с момента обеспечения надлежащего доступа к месту установки.'.
		'<p>2.5. Все необходимые материалы доставляются Исполнителем по адресу Заказчика указанному в пункте 1.4. настоящего Договора к моменту оказания услуг. В случае, отсутствия Заказчика или его представителя на объекте в согласованный день доставки, повторная доставка оплачивается из расчёта 1000 (одна тысяча) руб./заказ, при этом окончательный срок выполнения Договора составит 10 (десять) рабочих дней с момента обеспечения надлежащего доступа к месту установки.'.
		'<p>2.6. Исполнитель обязуется собрать строительный мусор в мешки предоставленные Заказчиком. Вынос и вывоз мусора, оставшегося после оказания услуг осуществляется силами Заказчика. Исполнитель обязуется вывезти строительный мусор на специализированную площадку (в соответствии с законом №239-29 от 29.05.2003), только в случае, если данная услуга была заказана Заказчиком и указана в Приложении №1.'.
		'<p>2.7. Заказчик обязуется оплатить полную стоимость услуг согласно условий пункта 3. настоящего Договора.'.
		'<p>2.8. Заказчик обязуется осуществить приёмку оказанных услуг по монтажу изделий, по количеству, качеству, комплектности, внешнему виду и качеству отделки подписав «Акт сдачи-приемки». При наличии со стороны Заказчика разногласий в «Акте сдачи-приемки» делаются соответствующие отметки. Экземпляр «Акта сдачи-приемки» Исполнителя передается представителю Исполнителя оказывающего услуги.'.
		'<p>2.9. В случае подписания Заказчиком «Акта сдачи–приемки» с разногласиями, Исполнитель обязуется рассмотреть данные разногласия в течение 5 (пяти) рабочих дней, при этом срок выполнения Договора автоматически продлевается на указанный срок.'.
		'<p>2.10. В случае согласия с разногласиями Заказчика Исполнитель обязан устранить замечания в течение 14 рабочих дней, с момента признания Исполнителем разногласий.'.
		'<p>2.11. После устранения Исполнителем разногласий Сторонами подписывается «Акт об устранении недостатков»'.
	'</div>'.

	'<div class="p-head">3. Стоимость услуг и порядок расчетов</div>'.
	'<div class="paragraph">'.
		'<p>3.1. Общая стоимость услуг по настоящему Договору составляет: '.$v['sum'].' ('._numToWord($v['sum']).' рубл'._end($v['sum'], 'ь', 'я', 'ей').') и не содержит в себе НДС. Исполнитель не является плательщиком НДС в соответствии с законодательством РФ.'.
		($v['avans'] ?
			'<p>3.2. Оплата по настоящему договору осуществляется в следующем порядке:'.
			'<p>3.2.1. Предоплата, вносимая Заказчиком в день заключения настоящего Договора составляет: '.$v['avans'].' ('._numToWord($v['avans']).' рубл'._end($v['avans'], 'ь', 'я', 'ей').')'.

			($v['dopl'] ?
				//'<p>3.2.2. (Без отсрочки) Оставшуюся сумму в размере: '.$dopl.' ('._numToWord($dopl).' рубл'._end($dopl, 'ь', 'я', 'ей').'), Заказчик оплачивает в кассу Исполнителя в день оказания услуг.'.
				//(С отсрочкой)
				'<p>3.2.2.  Оставшуюся сумму в размере: '.$v['dopl'].' ('._numToWord($v['dopl']).' рубл'._end($v['dopl'], 'ь', 'я', 'ей').'), Заказчик оплачивает в кассу Исполнителя до ______________________ 2017 г. включительно.'
			: '')
	...
	'<div class="p-head">4. Качество и гарантийные обязательства</div>'.
	'<div class="paragraph">'.
		'<p>4.1. Гарантийный срок на изделия ПВХ (оконные блоки) – 36 месяцев, на монтажные и отделочные работы по оконным блокам – 12 месяцев.'.
			'<p>Гарантийный срок на дверные блоки, роллетные системы и ворота – 12 месяцев. На монтажные и отделочные работы по установке дверных блоков, роллетных систем и ворот – 12 месяцев.'.
			'<p>Гарантийный срок действует с момента подписания сторонами «Акта сдачи – приемки». Для климатических условий Северо-западного и Центрального региона России рекомендовано использовать двухкамерные стеклопакеты.'.
			'<p>Заказчик предупрежден, что при установке однокамерного стеклопакета, возможно образование конденсата и промерзание стеклопакета в зимний период при более высокой температуре окружающей среды, чем при установке двухкамерного стеклопакета.'.
			'<p>Заказчик предупреждён, что для исключения возможности выпадения конденсата и образования наледи на стеклопакетах, необходимо поддержание уровня температуры и влажности, рекомендованного для жилого помещения.'.
		'<p>4.2. Исполнитель обязуется заменить входящие в состав Изделия, комплектующие за свой счёт, в случае выхода их из строя в течение гарантийного срока. Срок выполнения гарантийных работ составляет не более 20 (Двадцати) рабочих дней с момента поступления письменной претензии от Заказчика. Письменная претензия принимается в центральном офисе Исполнителя либо по почте, по адресу, указанному в пункте 9 настоящего Договора.'.
		'<p>4.3. Гарантия не распространяется на случаи, когда Изделие (или его комплектующие) утратили свои качественные характеристики вследствие неправильной эксплуатации, действий третьих лиц или в случае возникновения обстоятельств непреодолимой силы.'.
	'</div>'.

	'<div class="p-head">5. Ответственность сторон, форс-мажорные обстоятельства</div>'.
	'<div class="paragraph">'.
		'<p>5.1. Стороны освобождаются от ответственности за частичное или полное неисполнение обязательств по настоящему Договору, если это явилось следствием обстоятельств непреодолимой силы (форс-мажор), т.е. пожара, стихийных бедствий, войны, блокад, введение правительственных ограничений постфактум, объявления карантина и эпидемий. При этом срок исполнения обязательств по Договору продлевается на период действия указанных обстоятельств.'.
		'<p>5.2. За неисполнение или ненадлежащее исполнение обязательств Стороны несут ответственность в соответствии с действующим законодательством Российской Федерации.'.
		'<p>5.3. В случае нарушения сроков выполнения условий Договора Исполнитель выплачивает Заказчику неустойку в соответствии с Законом РФ "О защите прав потребителей.'.
		'<p>5.4. В случае нарушения сроков оплаты Заказчик выплачивает Исполнителю неустойку в соответствии с Законом РФ "О защите прав потребителей.'.
		'<p>5.5. Заказчик не вправе отказаться от приемки изделия изготовленного в соответствии с Приложением №1, являющимся неотъемлемой частью настоящего Договора.'.
		'<p>5.6. Стороны договорились о возможности расторжения Договора Заказчиком в любое время после заключения договора при условии компенсации Исполнителю 50% от стоимости услуг по Договору.'.
	'</div>'.
	...
	'<div class="p-head">9. Юридические адреса и банковские реквизиты сторон</div>'.
	'<table class="rekvisit">'.
		'<tr><td><center><b>ИСПОЛНИТЕЛЬ</b></center>'.
				'<br />'.
				'<center>«'._app('name').'»</center>'.
				'<br />'.
				'ИНН '._app('inn').' '.'КПП '._app('kpp').
				'<br />'.
				'ОГРН '._app('ogrn').
				'<br />'.
				'Юридический адрес: '._app('adres_yur').
				'<br />'.
				'Адрес для корреспонденции: '._app('adres_yur').
				'<br />'.
				'Адрес офиса: '._app('adres_ofice').
				'<br />'.
				'Тел. '._app('phone').
				'<br />'.
				'<br />'.
				'___________________ '._viewer(VIEWER_ID, 'viewer_name_init').
				'<div class="podpis">(подпись)</div>'.
				'<br />'.
				'<br />'.
				'М.П.'.
			'<td><center><b>ЗАКАЗЧИК:</b></center>'.
				'<br />'.
				'<center>'.$v['fio'].'</center>'.
				'<br />'.
				'Паспорт серии '.$v['pasp_seria'].' '.$v['pasp_nomer'].
				'<br />'.
				'выдан '.$v['pasp_ovd'].' '.$v['pasp_data'].
				'<br />'.
				'Адрес регистрации: '.$v['pasp_adres'].
				'<br />'.
				'<br />'.
				'____________________ '.$v['fio_podpis'].
				'<div class="podpis">(подпись)</div>'.
	'</table>');

	$doc->newPage();

	//АКТ сдачи-приёмки оказания услуг
	$doc->addParagraph(
		'<div class="act-ekz">Экземпляр Заказчика</div>'.
		'<center>'.
			'<b>АКТ сдачи-приёмки'.
				'<br />'.
				'к договору возмездного оказания услуг №'.$v['nomer'].' от '._dataDog($v['data_create']).
			'</b>'.
		'</center>'.
		'<br />'.
		'<div class="act-content">'.
			'Дата оказания услуг: ___________________________'.
			'<br />'.
			'Адрес выполнения работ: '.($v['adres'] ? ', '.$v['adres'] : '___________________________').
			'<br />'.
			'Заказ: №'.$v['nomer'].' Заказчик: '.$v['fio'].
			'<br />'.
			'<b>Экземпляр Заказчика является основанием для направления претензии.</b>'.
			'<br />'.
			'1. Конструкции из ПВХ (оконные блоки), дверные блоки, роллетные системы, гаражные и промышленные ворота <b>(нужное подчеркнуть)</b>'.
			'<br />'.
			'принял по количеству, качеству, комплектности и внешнему виду <b>(ненужное зачеркнуть)</b>:'.
			'<br />'.
			'- без замечаний,'.
			'<br />'.
			...
			'________________________________________________________________________________________'.
			'<br />'.
			'2. Выполненные работы принял <b>(не нужное зачеркнуть)</b>:'.
			'<br />'.
			'- без замечаний,'.
			'<br />'.
			'- со следующими замечаниями:'.
			...
			'<br />'.
			'<br />'.
			'<center>Услуги согласно условий договора выполнены полностью и в срок Заказчик претензий по объему качествам и срокам оказания услуг не имеет.</center>'.
		'</div>'.
		'<br />'.
		'<table class="act-podpis">'.
			'<tr><td>'.
					'Исполнитель'.
					'<br />'.
					'_______________/_________________'.
					'<div>(подпись)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(ф.и.о.)</div>'.
				'<td>'.
					'Заказчик'.
					'<br />'.
					'_______________/_________________'.
		...
		'<div class="act-ekz">Экземпляр Исполнителя</div>'.
		'<center>'.
			'<b>АКТ сдачи-приёмки'.
				'<br />'.
				'к договору возмездного оказания услуг №'.$v['nomer'].' от '._dataDog($v['data_create']).
...
function _zayavDogovorPrintContent2($doc, $v) {//Договор поставки товара
	if(TEMPLATE_2)
		return $doc;
	...
	'<div class="paragraph">'.
		'<p>Общество с ограниченной ответственностью «Территория Комфорта», '.
			...
			$v['fio'].($v['adres'] ? ', '.$v['adres'] : '').', именуемый(ая) в дальнейшем «Покупатель», с другой стороны, '.
			'совместно именуемые Стороны, заключили настоящий Договор поставки, в дальнейшем Договор, о нижеследующем:'.
	'</div>'.

	'<div class="p-head">1. Предмет договора</div>'.
	'<div class="paragraph">'.
		'<p>1.1. Продавец обязуется поставить конструкции из ПВХ (оконные блоки), дверные блоки, роллетные системы, гаражные и промышленные ворота (далее – Товар) Покупателю, а Покупатель обязуется принять и оплатить поставленный Товар.'.
		'<p>1.2. Комплектность Товара определяется в соответствии с Приложением №1, являющимся неотъемлемой частью настоящего Договора.'.
		'<p>1.3. Риск случайной гибели или случайного повреждения Товара несет Поставщик до момента принятия его Покупателем. Данный факт подтверждается подписанным Сторонами «Актом приема–передачи товара»).'.
		'<p>1.4. Право собственности на Товар по настоящему Договору переходит к Покупателю после подписания Сторонами «Акта приема – передачи товара»).'.
	'</div>'.

	'<div class="p-head">2. Условия поставки</div>'.
	'<div class="paragraph">'.
		'<p>2.1. Доставка Товара осуществляется (нужное подчеркнуть):'.
		'<p>- Покупателем самостоятельно'.
		'<p>- Продавцом по адресу: ____________________'.
		'<p>2.2. Продавец обязуется передать Покупателю Товар, а Покупатель - принять его в срок не позднее 30 (тридцати) рабочих дней после получения Продавцом предоплаты (п. 3 настоящего Договора).'.
		'<p>В случае заказа сложных и цветных изделий, срок Договора увеличивается на количество дополнительных дней на изготовление сложной конструкции, и указывается в Приложении №1.'.
	'</div>'.

	'<div class="p-head">3. Стоимость услуг и порядок расчетов</div>'.
	'<div class="paragraph">'.
		'<p>3.1. Стоимость Товара по настоящему Договору составляет: '.$v['sum'].' ('._numToWord($v['sum']).' рубл'._end($v['sum'], 'ь', 'я', 'ей').') и не содержит в себе НДС. Поставщик не является плательщиком НДС в соответствии с законодательством РФ.'.
		($v['avans'] ?
			'<p>3.2. Оплата по настоящему договору осуществляется в следующем порядке:'.
			'<p>3.2.1. Предоплата, вносимая Покупателем в день заключения настоящего Договора составляет: '.$v['avans'].' ('._numToWord($v['avans']).' рубл'._end($v['avans'], 'ь', 'я', 'ей').')'.
		($v['dopl'] ?
			//'<p>3.2.2. (Без отсрочки) Оставшуюся сумму в размере: '.$v['dopl'].' ('._numToWord($v['dopl']).' рубл'._end($v['dopl'], 'ь', 'я', 'ей').'), Покупатель оплачивает в кассу Поставщика в день передачи товара Покупателю.'.
			//(С отсрочкой)
			'<p>3.2.2. Оставшуюся сумму в размере: '.$v['dopl'].' ('._numToWord($v['dopl']).' рубл'._end($v['dopl'], 'ь', 'я', 'ей').'), Покупатель оплачивает в кассу Поставщика до ______________________ 2017 г. включительно.'
		: '')
	...
	'<div class="p-head">4. Качество и гарантийные обязательства</div>'.
	'<div class="paragraph">'.
		'<p>4.1. Качество Товара должно соответствовать техническим данным завода-изготовителя и образцам, продемонстрированным Покупателю Продавцом.'.
		'<p>4.2. Гарантийный срок на изделия ПВХ (оконные блоки) – 36 месяцев.'.
		'<p>Гарантийный срок на дверные блоки, роллетные системы и ворота – 12 месяцев.'.
		'<p>Гарантийный срок действует с момента подписания сторонами «Акта приема–передачи товара»). '.
		'<p>Для климатических условий Северо-западного и Центрального региона России рекомендовано использовать двухкамерные стеклопакеты.'.
		'<p>Покупатель предупреждён, что при установке однокамерного стеклопакета, возможно образование конденсата и промерзание стеклопакета в зимний период при более высокой температуре окружающей среды, чем при установке двухкамерного стеклопакета.'.
		'<p>Покупатель предупреждён, что для исключения возможности выпадения конденсата и образования наледи на стеклопакетах, необходимо поддержание уровня температуры и влажности рекомендованного для жилого помещения.'.
		'<p>4.3. Поставщик обязуется заменить входящие в состав Товара, комплектующие за свой счёт, в случае выхода их из строя в течение гарантийного срока. Срок выполнения гарантийных работ составляет не более 20 (Двадцати) рабочих дней с момента поступления письменной претензии от Покупателя. Письменная претензия принимается в центральном офисе Поставщика либо по почте, по адресу, указанному в пункте 9 настоящего Договора.'.
		'<p>4.4. Гарантия не распространяется на случаи, когда Товар (или его комплектующие) утратили свои качественные характеристики вследствие неправильной эксплуатации, действий третьих лиц или в случае возникновения обстоятельств непреодолимой силы.'.
	'</div>'.
	...
	'<div class="paragraph">'.
		'<p>5.1. Стороны освобождаются от ответственности за частичное или полное неисполнение обязательств по настоящему Договору, если это явилось следствием обстоятельств непреодолимой силы (форс-мажор), т.е. пожара, стихийных бедствий, войны, блокад, введение правительственных ограничений постфактум, объявления карантина и эпидемий. При этом срок исполнения обязательств по Договору продлевается на период действия указанных обстоятельств.'.
		'<p>5.2. За неисполнение или ненадлежащее исполнение обязательств Стороны несут ответственность в соответствии с действующим законодательством Российской Федерации.'.
		'<p>5.3. В случае нарушения сроков выполнения условий Договора Поставщик выплачивает Покупателю неустойку в соответствии с Законом РФ "О защите прав потребителей.'.
		'<p>5.4. В случае нарушения сроков оплаты Покупатель выплачивает Поставщику неустойку в соответствии с Законом РФ "О защите прав потребителей.'.
		'<p>5.5. Стороны договорились о возможности расторжения Договора Покупателем в любое время после заключения договора при условии компенсации Поставщику 50% от суммы Договора указанного в п.3 настоящего Договора.'.
	'</div>'.
	...
	'<div class="p-head">9. Юридические адреса и банковские реквизиты сторон</div>'.
	'<table class="rekvisit">'.
		'<tr><td><center><b>ПОСТАВЩИК</b></center>'.
				'<br />'.
				'<center>«'._app('name').'»</center>'.
				'<br />'.
				'ИНН '._app('inn').' '.'КПП '._app('kpp').
				...
				'<br />'.
				'М.П.'.
			'<td><center><b>ПОКУПАТЕЛЬ:</b></center>'.
				'<br />'.
				'<center>'.$v['fio'].'</center>'.
				'<br />'.
				'Паспорт серии '.$v['pasp_seria'].' '.$v['pasp_nomer'].
	...
	$doc->addParagraph(
		'<div class="act-ekz">Экземпляр Покупателя</div>'.
		'<center>'.
			'<b>АКТ приёма-передачи товара'.
				'<br />'.
				'к договору поставки №'.$v['nomer'].' от '._dataDog($v['data_create']).
			'</b>'.
		'</center>'.
		'<br />'.
		'<div class="act-content">'.
			'<p>Дата поставки товара: ___________________________'.
			'<br />'.
			'<br />'.
			'<p>Продавец: '._app('name').' в лице  менеджера по продажам, '._viewer(VIEWER_ID, 'viewer_name_full').', действующей на основании доверенности №7 от 05.04.2017г.'.
			'<p>Покупатель: '.$v['fio'].
			'<br />'.
			'(в дальнейшем вместе именуемые «Стороны» и по отдельности «Сторона»), составили настоящий Акт о нижеследующем:'.

			'<p>1.	В соответствии с п. 1 Договора между Сторонами №'.$v['nomer'].' от '._dataDog($v['data_create']).
			'<br />'.
			'Продавец передает, а Покупатель принимает Товар следующего ассортимента и количества:'.
			'<br />'.
			...
			'<br />'.
			'<br />'.
			'<p>2. Принятый Покупателем товар обладает качеством и ассортиментом, соответствующим требованиям Договора. Товар поставлен в установленные в Договоре сроки. Покупатель не имеет никаких претензий к принятому товару.'.
			'<br />'.
			'<br />'.
			'<p>3. Настоящий Акт составлен в двух экземплярах, имеющих равную юридическую силу, по одному экземпляру для каждой из Сторон и является неотъемлемой частью Договора между Сторонами.'.
		'</div>'.
		'<br />'.
		'<table class="act-podpis">'.
			'<tr><td>'.
		...
		'<div class="act-ekz">Экземпляр Поставщика</div>'.
		'<center>'.
			'<b>АКТ приёма-передачи товара'.
				'<br />'.
				'к договору поставки №'.$v['nomer'].' от '._dataDog($v['data_create']).
	...
	foreach($arr as $r) {
		...
		if($r['expense_id'])
			$dop =
				...
				'</b> '.
				$r['txt'].
				'<div class="fs11 color-pay mt3">Продублировано в расходах организации</div>';
