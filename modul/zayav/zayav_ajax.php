...
//		_salaryZayavBonus($z['id']);
	...
	case 'zayav_stat_load'://статистика по заявкам
		$service_id = _num($_POST['service_id']);
		...
		if($year_compare && $year_compare != $year)
			$send['series'][] = array(
				'name' => utf8(_service('count') == 1 ? 'Заявки' : _service('name', $service_id).($year_compare ? ' '.$year_compare : '')),
				'color' => '#ccc',
				'data' => _zayavStatYear($service_id, $year_compare, $zayav_ids)
			);

		$send['series'][] = array(
			'name' => utf8(_service('count') == 1 ? 'Заявки' : _service('name', $service_id).($year_compare ? ' '.$year : '')),
			'color' => '#86C4FF',
		...
		$send['html'] = utf8(
			'<table class="bs5 bor1 bg-gr1 w100p">'.
				'<tr'.(_service('count') == 1 ? ' class="dn"' : '').'>'.
					'<td class="label pb5 r">Вид деятельности:'.
					'<td><input type="hidden" id="filter-service" value="'.$service_id.'" />'.
				'<tr><td class="label pb5 r w125">Год:'.
					'<td><input type="hidden" id="filter-year" value="'.$year.'" />'.
				'<tr><td class="label pb5 r w125">Сравнить с годом:'.
...
function _zayavStatYear($service_id, $year, $zayav_ids) {//количество заявок за выбранный год
	//количество заявок по каждому месяцу за выбранный год
	$sql = "SELECT
