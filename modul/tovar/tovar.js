...
//			ven._select(vendor_id);
	...
	_tovarInventory = function() {
		var html =
				'<div class="_info">' +
					'<p>После нажатия кнопки "<b>Запустить инвентаризацию</b>" все товары, находящиеся в наличии, приобретают статус <u>под вопросом (под сомнением)</u>.' +
					'<p>После запуска инвентаризации в условиях поиска товаров появится фильтр <u>галочка - Инвентаризация не пройдена</u>, при котором будут выводиться только те товары, которые ещё не прошли инвентаризацию.' +
					...
					'<p>Товары, поступившие во время проведения инвентаризации, автоматически будут считаться прошедщими инвентаризацию.' +
				'</div>',
			dialog = _dialog({
				width:540,
