var _remindAdd=function(){var b=_dialog({top:40,width:480,head:"Внесение нового напоминания",content:'<table class="_remind-add-tab">'+(window.ZAYAV.id?'<tr><td class="label">Заявка:<td>'+ZAYAV.head:"")+(window.CLIENT?'<tr><td class="label">Клиент:<td>'+CLIENT.fio:"")+'<tr><td class="label">Задача:<td><input type="text" id="txt" /><tr><td class="label top">Подробно:<td><textarea id="about"></textarea><tr><td class="label">День выполнения:<td><input type="hidden" id="day" /></table><input type="hidden" id="client_id" value="'+
(window.CLIENT?CLIENT.id:0)+'"><input type="hidden" id="zayav_id" value="'+(window.ZAYAV.id?ZAYAV.id:0)+'">',submit:function(){var a={op:"remind_add",from:window.ZAYAV.id?"zayav":window.CLIENT?"client":"",client_id:$("#client_id").val(),zayav_id:$("#zayav_id").val(),txt:$.trim($("#txt").val()),about:$("#about").val(),day:$("#day").val()};a.txt?(b.process(),$.post(AJAX_MAIN,a,function(c){if(c.success)switch(b.close(),_msg("Напоминание внесено"),a.from){case "zayav":$("#_remind-zayav").html(c.html);
REMIND.active++;break;case "client":$("#remind-spisok").html(c.html);break;default:$("td.left").html(c.html)}else b.abort()},"json")):(b.err("Не указано содержание задачи"),$("#txt").focus())}});$("#txt").focus();$("#about").autosize();$("#day")._calendar();return!1},remindFilter=function(b,a){return{op:"remind_spisok",status:"_remind-status"==a?$("#_remind-status").val():$("#remind_filter_status").val()}},remindSpisok=function(b,a){$("#mainLinks").addClass("busy");$.post(AJAX_MAIN,remindFilter(b,
a),function(a){$("#mainLinks").removeClass("busy");a.success&&$(".left").html(a.html)},"json")};
$(document).on("click","._remind-add",_remindAdd).on("click","._remind-unit .action",function(){function b(a){var b={op:"remind_action",from:window.ZAYAV?"zayav":window.CLIENT?"client":"",id:c,status:a,day:$("#remind_day").val(),reason:$("#reason")._select("inp")};1==a&&b.day==d?e.err("Выберите другой день"):1!=a||b.reason?$.post(AJAX_MAIN,b,function(a){a.success&&(e.close(),_msg("Данные изменены!"),$(b.from?"#remind-spisok":"td.left").html(a.html))},"json"):e.err("Не указана причина")}for(var a=
$(this);!a.hasClass("_remind-unit");)a=a.parent();var c=a.attr("val"),f=a.find(".hdtxt").html(),d=a.find(".ruday").val(),e=_dialog({top:30,head:"Изменение статуса напоминания",content:'<div id="_remind-action-tab"><div id="hd">'+f+'</div><div class="st c1" val="1">Указать другой день<div class="about">Перенести напоминание на другой день.</div></div><div class="st c2" val="2">Выполнено<div class="about">Задание выполнено успешно.</div></div><div class="st c0" val="0">Отмена<div class="about">Отмена напоминания по какой-либо причине.</div></div><table id="ra-tab"><tr><td class="label">Новый день:<td><input type="hidden" id="remind_day" value="'+
