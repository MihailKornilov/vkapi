...
$(document).on("click","._remind-add",function(){var b=_dialog({top:40,width:480,head:"Внесение нового напоминания",content:'<table class="_remind-add-tab">'+(window.ZAYAV?'<tr><td class="label">Заявка:<td>'+ZAYAV.head:"")+(window.CLIENT?'<tr><td class="label">Клиент:<td>'+CLIENT.fio:"")+'<tr><td class="label">Задача:<td><input type="text" id="txt" /><tr><td class="label top">Подробно:<td><textarea id="about"></textarea><tr><td class="label">День выполнения:<td><input type="hidden" id="day" /></table><input type="hidden" id="client_id" value="'+
(window.CLIENT?CLIENT.id:0)+'"><input type="hidden" id="zayav_id" value="'+(window.ZAYAV?ZAYAV.id:0)+'">',submit:function(){var a={op:"remind_add",from:window.ZAYAV?"zayav":window.CLIENT?"client":"",client_id:$("#client_id").val(),zayav_id:$("#zayav_id").val(),txt:$.trim($("#txt").val()),about:$("#about").val(),day:$("#day").val()};a.txt?(b.process(),$.post(AJAX_MAIN,a,function(d){d.success?(b.close(),_msg("Напоминание внесено"),$(a.from?"#remind-spisok":"td.left").html(d.html),"zayav"==a.from&&window.REMIND&&
REMIND.active++):b.abort()},"json")):(b.err("Не указано содержание задачи"),$("#txt").focus())}});$("#txt").focus();$("#about").autosize();$("#day")._calendar();return!1}).on("click","._remind-unit .action",function(){function b(a){var b={op:"remind_action",from:window.ZAYAV?"zayav":window.CLIENT?"client":"",id:d,status:a,day:$("#remind_day").val(),reason:$("#reason")._select("inp")};1==a&&b.day==c?e.err("Выберите другой день"):1!=a||b.reason?$.post(AJAX_MAIN,b,function(a){a.success&&(e.close(),_msg("Данные изменены!"),
$(b.from?"#remind-spisok":"td.left").html(a.html))},"json"):e.err("Не указана причина")}for(var a=$(this);!a.hasClass("_remind-unit");)a=a.parent();var d=a.attr("val"),f=a.find(".hdtxt").html(),c=a.find(".ruday").val(),e=_dialog({top:30,head:"Изменение статуса напоминания",content:'<div id="_remind-action-tab"><div id="hd">'+f+'</div><div class="st c1" val="1">Указать другой день<div class="about">Перенести напоминание на другой день.</div></div><div class="st c2" val="2">Выполнено<div class="about">Задание выполнено успешно.</div></div><div class="st c0" val="0">Отмена<div class="about">Отмена напоминания по какой-либо причине.</div></div><table id="ra-tab"><tr><td class="label">Новый день:<td><input type="hidden" id="remind_day" value="'+
...
e.butSubmit("Применить")):($("#_remind-action-tab").addClass("busy"),b(a))})}).on("click","._remind-unit .hd-edit",function(){function b(){var a={op:"remind_head_edit",from:window.ZAYAV?"zayav":window.CLIENT?"client":"",id:d,txt:$("#hdtxt").val(),about:$("#hd-about").val()};a.txt?(c.process(),$.post(AJAX_MAIN,a,function(b){b.success?(c.close(),_msg("Данные изменены"),$(a.from?"#remind-spisok":"td.left").html(b.html)):c.abort()},"json")):c.err("Не указано содержание задачи")}for(var a=$(this);!a.hasClass("_remind-unit");)a=
a.parent();var d=a.attr("val"),f=a.find(".hdtxt").html(),a=a.find(".hd-about").html().replace(/<br>/g,""),c=_dialog({width:420,head:"Редактирование содержания напоминания",content:'<table id="_remind-head-edit"><tr><td class="label">Задача:<td><input type="text" id="hdtxt" value="'+f+'" /><tr><td class="label top">Подробно:<td><textarea id="hd-about">'+a+"</textarea></table>",butSubmit:"Сохранить",submit:b});$("#hdtxt").focus().keyEnter(b);$("#hd-about").autosize()}).on("click","._remind-unit .ruhist",
