...
b=c+1,d=[];a=_num(a);2E3<a&&a<c&&(c=a);a>b&&(b=a);for(a=c;a<=b;a++)d[a]=a;return d},_checkAll=function(a,c){for(var b=$("._check"),d=b.length,e=[],h=0;h<d;h++){var f=b.eq(h),g=_num(f.attr("id").split("_check")[0].split("ch")[1]);g&&("change"==a?$("#ch"+g)._check(c):_num(f.find("input").val())&&e.push(g))}return"array"==a?e:e.join()};
...
h.attr("src").replace("-s.","-b.")+'"></table><div class="about"><div class="dtime"></div></div><div class="hide"></div></div>';FB.append(h);var g=$("#_image-view"),n=[],m=1;e();g.find(".head a").click(d);g.find("img:first").on("load",e);$.post(AJAX_MAIN,{op:"image_view",id:a},function(a){g.find("._busy").removeClass("_busy");a.success&&(n=a.img,m=a.n,c(),b())},"json")}).on("click","._next",function(){var a=$(this),c=a.attr("val");c&&(c=c.split(":"),c[1]&&!a.hasClass("_busy")&&(a.addClass("_busy"),
window[c[0]].page=c[1],$.post(AJAX_MAIN,window[c[0]],function(b){b.success?(a.after(b.spisok).remove(),_nextCallback()):a.removeClass("_busy")},"json")))}).ready(function(){VK.callMethod("scrollWindow",0);VK.callMethod("scrollSubscribe");VK.addCallback("onScroll",function(a){VK_SCROLL=a;for(var c in _onScroll)_onScroll[c](a)});FB=$("#frameBody");FH=$("#frameHidden");_fbhs();sortable();$(".pagehelp_create").click(function(){var a=$(this).attr("val"),c=_dialog({top:10,width:610,head:"Создание новой подсказки",
content:'<TABLE class="pagehelp_tab"><TR><TD class="label">Страница:<TD><b>'+a+'</b><TR><TD class="label">Название:<TD><input type="text" id="name" maxlength="200"><TR><TD class="label">Содержание:<TD><TR><td colspan="2"><textarea id="pagehelp_txt"></textarea></TABLE>',submit:function(){var b={op:"pagehelp_add",page:a,name:$("#name").val(),txt:$("#pagehelp_txt").val()};b.name?(c.process(),$.post(AJAX_MAIN,b,function(a){a.success?(c.close(),_msg("Внесёно!"),document.location.reload()):c.abort()},"json")):
(c.bottom.vkHint({msg:'<SPAN class="red">Не введено название</SPAN>',remove:1,indent:40,show:1,top:-48,left:217}),$("#name").focus())}});$("#name").focus();$("#pagehelp_txt").autosize()});$("#mainLinks .img_pagehelp").click(function(){function a(a){return'<div class="headName">'+a.name+a.edit+'</div><div class="pagehelp_show_txt">'+a.txt+a.dtime+"</div>"}function c(a){e=_dialog({top:10,width:610,head:"Информация о странице",load:a?0:1,content:a,butSubmit:"",butCancel:"Закрыть"})}function b(f){e.close();
e=_dialog({top:10,width:610,head:"Редактирование подсказки",content:'<TABLE class="pagehelp_tab"><TR><TD class="label">Страница:<TD><b>'+f.page+'</b><TR><TD class="label">Название:<TD><input type="text" id="name" maxlength="200" value="'+f.name+'"><TR><TD class="label">Содержание:<TD><TR><td colspan="2"><textarea id="pagehelp_txt">'+f.txt+"</textarea></TABLE>",butSubmit:"Сохранить",submit:function(){var g={op:"pagehelp_edit",id:d,name:$("#name").val(),txt:$("#pagehelp_txt").val()};g.name?(e.process(),
$.post(AJAX_MAIN,g,function(){f.success?(e.close(),f.name=g.name,f.txt=g.txt,c(a(f)),e.content.find(".add").click(function(){b(f)})):e.abort()},"json")):(e.bottom.vkHint({msg:'<SPAN class="red">Не введено название</SPAN>',remove:1,indent:40,show:1,top:-48,left:217}),$("#name").focus())}});$("#name").focus();$("#pagehelp_txt").autosize()}var d=$(this).attr("val"),e,h={op:"pagehelp_get",id:d};c();$.post(AJAX_MAIN,h,function(c){c.success?e.content.html(a(c)).find(".add").click(function(){b(c)}):e.loadError()},
"json")});imageSortable();window.frameHidden.onresize=_fbhs;$("#pin-enter").length&&($("#pin").focus().keydown(function(){$(".red").html("&nbsp;")}).keyEnter(pinEnter),$(".vkButton").click(pinEnter))});
