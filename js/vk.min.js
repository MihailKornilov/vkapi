...
0==d&&(DIALOG_MAXHEIGHT=0);var f=$("body").append(e).find("._dialog:last"),e=f.find(".content"),g=f.find(".bottom"),h=g.find(".submit"),m=Math.round(a.width/2);f.find(".head .img_del").click(c);h.click(function(){a.submit()});g.find(".cancel").click(function(b){b.stopPropagation();a.cancel();c()});e.find("input").keyEnter(a.submit);_backfon();f.css({width:a.width+"px",top:$(window).scrollTop()+VK_SCROLL+a.top+"px",left:313-m+"px","z-index":ZINDEX+5});window["dFrame"+d].onresize=function(){for(var b=
...
spisok:[],light:0,right:15,block:1,func:function(){}},a);var s=_num(e.val(),1);e.wrap('<div class="_radio'+(a.block?" block":"")+'" id="'+g+'_radio">');b(_copySel(a.spisok));d(a.func);e.spisokUpdate=function(a){e.parent().find("div").remove();b(a)};return window[h]=e};
