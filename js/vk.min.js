...
'">'+a.butSubmit+"</button>"+(a.butCancel?'<button class="vk cancel">'+a.butCancel+"</button>":"")+"</div></div>";0==d&&(DIALOG_MAXHEIGHT=0);var f=$("body").append(e).find("._dialog:last"),e=f.find(".content"),g=f.find(".bottom"),l=g.find(".submit"),m=Math.round(a.width/2);f.find(".head .img_del").click(c);l.click(function(){a.submit()});g.find(".cancel").click(function(b){b.stopPropagation();a.cancel();c()});e.find("input").keyEnter(a.submit);_backfon();f.css({width:a.width+"px",top:$(window).scrollTop()+
VK_SCROLL+a.top+"px",left:313-m+"px","z-index":ZINDEX+5});window["dFrame"+d].onresize=function(){for(var b=$(".dFrame"),c=0,d=0;d<b.length;d++){var e=b.eq(d).height();e>c&&(c=e)}b=c+VK_SCROLL+80+a.mb;DIALOG_MAXHEIGHT!=b&&(DIALOG_MAXHEIGHT=b,_fbhs())};return{close:c,process:function(){l.addClass("_busy")},abort:function(a){l.removeClass("_busy");a&&b(a)},bottom:g,content:e,err:b,loadError:function(){f.find(".load").removeClass("_busy")},butSubmit:function(a){l[(a?"remove":"add")+"Class"]("dn");l.html(a)},
submit:function(b){a.submit=b}}},_dialogDel=function(a){a=$.extend({id:0,head:"записи",info:"",op:"_del",func:function(){}},a);var c=_dialog({head:"Удаление "+a.head,padding:30,content:(a.info?'<div class="_info">'+a.info+"</div>":"")+'<center class="red">Подтвердите удаление<br /><b>'+a.head+"</b>.</center>",butSubmit:"Удалить",submit:function(){var b={op:a.op,id:a.id};c.process();$.post(AJAX_MAIN,b,function(b){b.success?(c.close(),_msg("Удалено"),a.func(b)):(c.abort(),c.err(b.text))},"json")}})},
_tooltip=function(a,c,b){return' _tooltip"><div class="ttdiv"'+(c?' style="left:'+c+'px"':"")+'><div class="ttmsg">'+a+'</div><div class="ttug'+(b?" "+b:"")+'"></div></div>'},_parent=function(a,c){c=c||"TR";var b=10,d=0,e="."==c[0];for(e&&(c=c.substr(1));!d;)a=a.parent(),d=e?a.hasClass(c):a[0].tagName==c,--b||(d=1);return a},_busy=function(a){var c=$("#_menu");if(0===a)c.removeClass("_busy");else{if(c.hasClass("_busy"))return!0;c.addClass("_busy")}},_filterSpisok=function(a,c,b){b&&(a[b]=c);a.page=
1;_scroll("clear");c=a.op.split("_spisok")[0];for(var d in a)"op"!=d&&"page"!=d&&"type_id"!=d&&_cookie(COOKIE_PREFIX+c+"_"+d,escape(a[d]))},_nextCallback=function(){},_yearAss=function(a){var c=(new Date).getFullYear(),b=c+1,d=[];a=_num(a);2E3<a&&a<c&&(c=a);a>b&&(b=a);for(a=c;a<=b;a++)d[a]=a;return d},_checkAll=function(a,c){for(var b=$("._check"),d=b.length,e=[],f=0,g=[],l=0;l<d;l++){var m=b.eq(l),s=_num(m.attr("id").split("_check")[0].split("ch")[1]);s&&("change"==a?$("#ch"+s)._check(c):_num(m.find("input").val())&&
(e.push(s),"sum"==a&&(f+=1*_parent(m).find(".sum").html()),g.push(m.parent().attr("val"))))}return"array"==a?e:"count"==a?e.length:"sum"==a?f:"type"==a?g.join():e.join()};
...
' class="clear'+_tooltip("Очистить",-49,"r")+"</div>":"")+(a.funcAdd?'<td class="seladd">':"")+'<td class="selug"></table><div class="selres" style="width:'+a.width+'px"></div></div>';n.next().remove("._select");n.after(q);var x=n.next(),t=x.find(".selinp"),D=x.find(".clear"),G=x.find(".selsel"),y=x.find(".selres"),B,A=x.find(".title0bg"),z,E=[],F={},w=0,H=/(<[\/]?[_a-zA-Z0-9=\"' ]*>)/i,I={38:1,40:1,13:1,27:1,9:1};d();if(a.multiselect){if(0!=r)for(q=r.split(","),h=0;h<q.length;h++)F[q[h]]=!0,t.before('<div class="multi">'+
z[q[h]]+'<span class="x" val="'+q[h]+'"></span></div>');l()}a.funcAdd&&!a.disabled&&x.find(".seladd").click(function(){a.funcAdd(p)});c();f(r);var C=t.val();a.disabled||($(document).off("click","#"+p+"_select .selug").on("click","#"+p+"_select .selug",m).off("click","#"+p+"_select .selsel").on("click","#"+p+"_select .selsel",function(){t.focus()}).off("click","#"+p+"_select .selun").on("click","#"+p+"_select .selun",function(){g($(this))}).off("mouseenter","#"+p+"_select .selun").on("mouseenter",
