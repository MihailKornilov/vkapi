...
$.fn._calendar=function(a){function c(){var c,d="<tr>",e=s.getFullYear(),f=s.getMonth()+1,g=s.getDate(),m;m=(new Date(a.year,a.mon-1,1)).getDay();m=0==m?7:m;var l=e==a.year&&f==a.mon,p=0==a.lost,r=a.year==n&&a.mon==h,u;c=a.year;u=a.mon;u--;0==u&&(u=12,c--);u=32-(new Date(c,u,32)).getDate();if(1<m)for(c=0;c<m-1;c++)d+="<td>";for(c=1;c<=u;c++){var w="";a.year<e?w=" lost":a.year==e&&a.mon<f?w=" lost":a.year==e&&a.mon==f&&c<g&&(w=" lost");d+='<td class="'+(!w||w&&!p?" sel":"")+(l&&c==g?" b":"")+(r&&c==
q?" set":"")+w+'"'+(!w||w&&!p?' val="'+c+'"':"")+">"+c;m++;8==m&&c!=u&&(d+="<tr>",m=1)}v.html(d).find(".sel").click(b)}function b(){n=a.year;h=a.mon;q=$(this).attr("val");p.html(q+" "+MONTH_DAT[h]+" "+n);g.val(d());a.func(d())}function d(){return n+"-"+(10>h?"0":"")+h+"-"+(10>q?"0":"")+q}function e(b){b.stopPropagation();a.mon--;0==a.mon&&(a.mon=12,a.year--);u.html(MONTH_DEF[a.mon]+" "+a.year);c()}function f(b){b.stopPropagation();a.mon++;13==a.mon&&(a.mon=1,a.year++);u.html(MONTH_DEF[a.mon]+" "+
a.year);c()}var g=$(this),l=g.attr("id"),m=g.val(),s=new Date;a=$.extend({year:s.getFullYear(),mon:s.getMonth()+1,day:s.getDate(),lost:0,func:function(){},place:"right"},a);REGEXP_DATE.test(m)&&"0000-00-00"!=m&&(m=m.split("-"),a.year=m[0],a.mon=Math.abs(m[1]),a.day=Math.abs(m[2]));g.wrap('<div class="_calendar" id="'+l+'_calendar">');g.after('<div class="calinp">'+a.day+" "+MONTH_DAT[a.mon]+" "+a.year+'</div><div class="calabs"></div>');var n=a.year,h=a.mon,q=a.day,p=g.next(),r=p.next(),u,v;g.val(d());
p.click(function(b){if(!r.html()){b.stopPropagation();b=$(".calabs");for(var d=0;d<b.length;d++){var m=b.eq(d);m.parent().attr("id").split("_calendar")[0]!=l&&m.html("")}$(document).on("click.calendar"+l,function(){r.html("");$(document).off("click.calendar"+l)});a.year=n;a.mon=h;a.day=q;r.html('<div class="calcal" style="left:'+("right"==a.place?0:-64)+'px"><table class="calhead"><tr><td class="calback"><td class="calmon">'+MONTH_DEF[h]+" "+n+'<td class="calnext"></table><table class="calweeks"><tr><td>Пн<td>Вт<td>Ср<td>Чт<td>Пт<td>Сб<td>Вс</table><table class="caldays"></table></div>');
...
$.fn._dropdown=function(a){function c(c){b();a.nosel||(d.val(c),p.html(l[c])[(a.title0&&!c?"add":"remove")+"Class"]("grey"),u.html(l[c]))}function b(){h&&(clearTimeout(h),h=0)}var d=$(this),e=d.attr("id");if("number"==typeof a||"string"==typeof a){switch(a){case "remove":d.next().remove("._dropdown");break;default:window[e+"_dropdown"].value(a)}return d}a=$.extend({head:"",headgrey:0,disabled:0,title0:"",spisok:[],func:function(){},nosel:0},a);var f,g=1*d.val()||0,l=function(){for(var b=a.title0?
{0:a.title0}:{},c=0;c<a.spisok.length;c++){var d=a.spisok[c];b[d.uid]=d.title}return b}(),m=a.head||a.title0,s=a.spisok.length,n=a.title0&&!a.disabled?'<a class="ddu grey'+(s?"":" last")+(g?"":" seld")+'" val="0">'+a.title0+"</a>":"",h=0;d.val(g);for(f=0;f<s;f++){var q=a.spisok[f],n=n+('<a class="ddu'+(f==s-1?" last":"")+(g==q.uid?" seld":"")+'" val="'+q.uid+'">'+q.title+"</a>");g==q.uid&&(m=q.title)}d.next().remove("._dropdown");d.after('<div class="_dropdown'+(a.disabled?" disabled":"")+'" id="'+
...
name:$.trim($("#name").val())};b.name?(d.process(),$.post(AJAX_MAIN,b,function(b){d.abort();b.success&&(d.close(),l.addClass("ex"),e.val(f),ATTACH[f]=b.arr,c(),a.func(f))},"json")):d.err("Не указано название файла")}}),g=$("#attach-add-tab"),h=g.find(".vk"),q=g.find("form"),p=$("#file"),r;p.change(function(){p.css("visibility","hidden");h.addClass("_busy");_cookie("_attached",0);_cookie("_attached_id",0);r=setInterval(b,500);q.submit()})});c(f)};
...
0!=b.correct&&$(document).off("keydown.hint");switch(h){case "wait_to_hidding":clearTimeout(q);h="show";break;case "hidding":h="showing";l.stop().animate({top:s,left:n,opacity:1},200,c);break;case "hidden":0<b.delayShow?(h="wait_to_showing",q=setTimeout(a,b.delayShow)):a()}}function e(){function d(){h="hidding";l.animate({opacity:"hide"},200,function(){h="hidden";0!=b.remove&&(g.remove(),a.off(b.event+".hint"),a.off("mouseleave.hint"))})}0!=b.correct&&$(document).off("keydown.hint");"wait_to_showing"==
h&&(clearTimeout(q),h="hidden");"showing"==h&&(l.stop(),d());"show"==h&&(0<b.delayHide?(h="wait_to_hidding",q=setTimeout(d,b.delayHide)):d())}b=$.extend({msg:"Сообщение подсказки",width:0,event:"mouseenter",ugol:"bottom",indent:"center",top:0,left:0,show:0,delayShow:0,delayHide:0,correct:0,remove:0},b);var f="<TABLE class=cont_table><TR><TD class=ugttd colspan=3>"+("top"==b.ugol?"<div class=ugt></div>":"")+"<TR><TD class=ugltd>"+("left"==b.ugol?"<div class=ugl></div>":"")+"<TD class=cont>"+(1==b.correct?
...
a.off("mouseleave.hint");a.on(b.event+".hint",d);a.on("mouseleave.hint",e);l.on("mouseenter.hint",d);l.on("mouseleave.hint",e);var h="hidden",q=0;0!=b.show&&d()};$.fn.vkHint=function(c){return new a($(this),c)}})();
$.fn._select=function(a){function c(){if(a.spisok.length){if(a.write){var b=t.val();if(b&&b.length){var c=[];reg=RegExp(b,"i");for(h=0;h<a.spisok.length;h++){for(var b=a.spisok[h],d=b.content.split(H),e=0;e<d.length;e++){var f=d[e];if(f.length&&!H.test(f)&&reg.test(f)){d[e]=f.replace(reg,"<em>$&</em>");b.content=d.join("");c.push(b);break}}if(a.limit&&c.length==a.limit)break}a.spisok=c}}c=a.title0&&!a.write?'<div class="selun title0" val="0">'+a.title0+"</div>":"";b=a.spisok.length;a.limit&&b>a.limit&&
(b=a.limit);for(h=0;h<b;h++)d=a.spisok[h],F[d.uid]||(c+='<div class="selun" val="'+d.uid+'">'+(d.content||d.title)+"</div>");y.removeClass("h250").html(c).find(".selun:last").addClass("last");A=y.height();250<A&&y.addClass("h250")}else y.html('<div class="nofind">'+a.nofind+"</div>").removeClass("h250")}function b(a){if(I[a.keyCode]){a.preventDefault();var b=y.find(".selun"),c=y[0],d=b.length;for(h=0;h<d&&!b.eq(h).hasClass("ov");h++);switch(a.keyCode){case 38:h==d&&(h=1);0<h?(1<d&&b.eq(h).removeClass("ov"),
...
break;case 27:case 9:t.blur(),s()}}}function d(){z=a.title0?{0:""}:{};for(h=0;h<a.spisok.length;h++){var b=a.spisok[h];z[b.uid]=b.title;b.content||(b.content=b.title);E.push({uid:b.uid,title:b.title,content:b.content})}}function e(){a.spisok=[];for(h=0;h<E.length;h++){var b=E[h];a.spisok.push({uid:b.uid,title:b.title,content:b.content})}c()}function f(b){if(a.multiselect)if(w){b=G.find(".x");var c=[];for(h=0;h<b.length;h++)c.push(b.eq(h).attr("val"));n.val(c.join())}else n.val(0);else if(r=b,n.val(b),
D[b?"show":"hide"](),b||!b&&!a.write_save)t.val(z[b]),B[0==b?"show":"hide"]()}function g(b){b=parseInt(b.attr("val"));var c;a.multiselect&&((!a.title0&&!b||0<b)&&t.before('<div class="multi">'+z[b]+'<span class="x" val="'+b+'"></span></div>'),l(b,!0));f(b);for(h=0;h<a.spisok.length;h++){var d=a.spisok[h];if(d.uid==b){c=d;break}}a.func(b,p,c);C=t.val()}function l(b,d){var e=G.find(".multi"),f=0;w=e.length;for(h=0;h<w;h++){var g=e.eq(h).width();f+g>v+4&&(f=0);f+=g+5+2}f=v-f;t.width(25>f?v:f);void 0!==
b&&(F[b]=d,c(),(!a.title0&&0==b||0<b)&&t.val(""));B[w?"hide":"show"]()}function m(){if(!x.hasClass("rs")){x.addClass("rs");var a=y.find(".selun"),c=y[0];a.removeClass("ov");for(h=0;h<a.length;h++){var d=a.eq(h);if(d.attr("val")==r){d.addClass("ov");d=d[0];a=d.offsetTop+d.offsetHeight;170<a&&(d=250,A>a&&(d-=120<A-a?120:A-a),c.scrollTop=a-d);break}}$(document).on("click."+p+"_select",s).on("keydown."+p+"_select",b)}}function s(){t.is(":focus")||(x.removeClass("rs"),a.write&&!r&&(t.val()&&!a.write_save&&
(t.val(""),a.funcKeyup("")),f(0),a.func(0,p)),$(document).off("click."+p+"_select").off("keydown."+p+"_select"))}var n=$(this),h,q,p=n.attr("id"),r=n.val()||0;if(p){switch(typeof a){default:case "number":case "string":q=window[p+"_select"];switch(a){case "process":q.process();break;case "cancel":q.cancel();break;case "title":return q.title();case "inp":return q.inp();case "focus":q.focus();break;case "remove":$("#"+p+"_select").remove();window[p+"_select"]=null;break;default:if(REGEXP_NUMERIC_MINUS.test(a)){var u=
q.o.write_save;q.o.write_save=0;q.value(a);q.o.write_save=u}}return n;case "object":if("length"in a)return q=window[p+"_select"],q.spisok(a),n;if(!("spisok"in a))return n}a=$.extend({width:150,disabled:0,block:!1,bottom:0,title0:"",spisok:[],limit:0,write:0,write_save:0,nofind:"Список пуст",multiselect:0,func:function(){},funcAdd:null,funcKeyup:e},a);a.clear=a.write&&!a.multiselect;a.multiselect&&(a.write=!0);var v=a.width-17-5-4;a.funcAdd&&(v-=18);a.clear&&(v-=17,r=_num(r));q='<div class="_select'+
(a.disabled?" disabled":"")+'" id="'+p+'_select" style="width:'+a.width+"px"+(a.block?";display:block":"")+(a.bottom?";margin-bottom:"+a.bottom+"px":"")+'"><div class="title0bg">'+a.title0+'</div><table class="seltab"><tr><td class="selsel"><input type="text" class="selinp" style="width:'+v+"px"+(a.write&&!a.disabled?"":";cursor:default")+'"'+(a.write&&!a.disabled?"":" readonly")+" />"+(a.clear?"<div"+(r?"":' style="display:none"')+' class="img_del'+_tooltip("Очистить",-49,"r")+"</div>":"")+(a.funcAdd?
'<td class="seladd">':"")+'<td class="selug"></table><div class="selres" style="width:'+a.width+'px"></div></div>';n.next().remove("._select");n.after(q);var x=n.next(),t=x.find(".selinp"),D=x.find(".img_del"),G=x.find(".selsel"),y=x.find(".selres"),A,B=x.find(".title0bg"),z,E=[],F={},w=0,H=/(<[\/]?[_a-zA-Z0-9=\"' ]*>)/i,I={38:1,40:1,13:1,27:1,9:1};d();if(a.multiselect){if(0!=r)for(q=r.split(","),h=0;h<q.length;h++)F[q[h]]=!0,t.before('<div class="multi">'+z[q[h]]+'<span class="x" val="'+q[h]+'"></span></div>');
l()}a.funcAdd&&!a.disabled&&x.find(".seladd").click(function(){a.funcAdd(p)});c();f(r);var C=t.val();a.disabled||($(document).off("click","#"+p+"_select .selug").on("click","#"+p+"_select .selug",m).off("click","#"+p+"_select .selsel").on("click","#"+p+"_select .selsel",function(){t.focus()}).off("click","#"+p+"_select .selun").on("click","#"+p+"_select .selun",function(){g($(this))}).off("mouseenter","#"+p+"_select .selun").on("mouseenter","#"+p+"_select .selun",function(){y.find(".ov").removeClass("ov");
$(this).addClass("ov")}).off("click","#"+p+"_select .x").on("click","#"+p+"_select .x",function(b){b.stopPropagation();b=$(this).attr("val");$(this).parent().remove();l(b,!1);f(b);a.func(b,p)}),t.focus(function(){m();a.write&&B.css("color","#ccc")}).blur(function(){a.write&&B.css("color","#888")}).keyup(function(b){I[b.keyCode]||(B[t.val()||w?"hide":"show"](),D[t.val()?"show":"hide"](),C!=t.val()&&(C=t.val(),a.funcKeyup(C),n.val(0),r=0))}),D.click(function(b){b.stopPropagation();f(0);a.func(0,p)}));
n.o=a;n.value=f;n.process=function(){t.addClass("_busy")};n.cancel=function(){t.removeClass("_busy")};n.spisok=function(b){a.spisok=b;d();c();n.cancel()};n.title=function(){return z[n.val()]};n.inp=function(){return t.val()};n.focus=function(){t.focus()};return window[p+"_select"]=n}};
