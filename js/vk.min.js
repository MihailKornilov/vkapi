...
$.fn._radio=function(a,c){function b(b){var c="";a.title0&&b.unshift([{uid:0,title:a.title0}]);for(f=0;f<b.length;f++)var d=b[f],c=c+('<div class="'+(s==d.uid?"on":"off")+(a.light?" l":"")+'" val="'+d.uid+'"'+(a.right?' style="margin-right:'+a.right+'px"':"")+"><s></s>"+d.title+"</div>");e.after(c)}function d(a){$(document).off("click","#"+g+"_radio .on,#"+g+"_radio .off");$(document).on("click","#"+g+"_radio .on,#"+g+"_radio .off",function(){a(_num(e.val()),g)})}var e=$(this),f,g=e.attr("id"),h=
g+"_radio";switch(typeof a){case "number":case "string":if("spisokUpdate"==a){window[h].spisokUpdate(c);return}h=e.parent();if(h.hasClass("_radio")){h.find(".on").removeClass("on").addClass("off");h=h.find("div");for(f=0;f<h.length;f++){var m=h.eq(f);if(a==m.attr("val")){m.addClass("on").removeClass("off");break}}e.val(a)}return e;case "function":return d(a),e;case "object":if("length"in a){h=$("#"+g+"_radio").find("div");for(f=0;f<h.length;f++)m=h.eq(f),m.html("<s></s>"+a[f].title);return e}}a=$.extend({title0:"",
spisok:[],light:0,right:15,block:1,func:function(){}},a);var s=e.val();e.wrap('<div class="_radio'+(a.block?" block":"")+'" id="'+g+'_radio">');b(a.spisok);d(a.func);e.spisokUpdate=function(a){e.parent().find("div").remove();console.log(a);b(a)};return window[h]=e};
