...
break;case 27:case 9:t.blur(),s()}}}function d(){A=a.title0?{0:""}:{};for(h=0;h<a.spisok.length;h++){var b=a.spisok[h];A[b.uid]=b.title;b.content||(b.content=b.title);E.push({uid:b.uid,title:b.title,content:b.content})}}function e(){a.spisok=[];for(h=0;h<E.length;h++){var b=E[h];a.spisok.push({uid:b.uid,title:b.title,content:b.content})}c()}function f(b){if(a.multiselect)if(w){b=G.find(".x");var c=[];for(h=0;h<b.length;h++)c.push(b.eq(h).attr("val"));n.val(c.join())}else n.val(0);else if(r=b,n.val(b),
D[b?"show":"hide"](),b||!b&&!a.write_save)t.val(A[b]),z[0==b?"show":"hide"]()}function g(b){b=parseInt(b.attr("val"));var c;a.multiselect&&((!a.title0&&!b||0<b)&&t.before('<div class="multi">'+A[b]+'<span class="x" val="'+b+'"></span></div>'),l(b,!0));f(b);for(h=0;h<a.spisok.length;h++){var d=a.spisok[h];if(d.uid==b){c=d;break}}a.func(b,p,c);C=t.val()}function l(b,d){var e=G.find(".multi"),f=0;w=e.length;for(h=0;h<w;h++){var g=e.eq(h).width();f+g>v+4&&(f=0);f+=g+5+2}f=v-f;t.width(25>f?v:f);void 0!==
b&&(F[b]=d,c(),(!a.title0&&0==b||0<b)&&t.val(""));z[w?"hide":"show"]()}function m(){if(!x.hasClass("rs")){x.addClass("rs");var a=y.find(".selun"),c=y[0];a.removeClass("ov");for(h=0;h<a.length;h++){var d=a.eq(h);if(d.attr("val")==r){d.addClass("ov");d=d[0];a=d.offsetTop+d.offsetHeight;170<a&&(d=250,B>a&&(d-=120<B-a?120:B-a),c.scrollTop=a-d);break}}$(document).on("click."+p+"_select",s).on("keydown."+p+"_select",b)}}function s(){t.is(":focus")||(x.removeClass("rs"),a.write&&!r&&(t.val()&&!a.write_save&&
...
'<td class="seladd">':"")+'<td class="selug"></table><div class="selres" style="width:'+a.width+'px"></div></div>';n.next().remove("._select");n.after(q);var x=n.next(),t=x.find(".selinp"),D=x.find(".img_del"),G=x.find(".selsel"),y=x.find(".selres"),B,z=x.find(".title0bg"),A,E=[],F={},w=0,H=/(<[\/]?[_a-zA-Z0-9=\"' ]*>)/i,I={38:1,40:1,13:1,27:1,9:1};d();if(a.multiselect){if(0!=r)for(q=r.split(","),h=0;h<q.length;h++)F[q[h]]=!0,t.before('<div class="multi">'+A[q[h]]+'<span class="x" val="'+q[h]+'"></span></div>');
...
$(this).addClass("ov")}).off("click","#"+p+"_select .x").on("click","#"+p+"_select .x",function(b){b.stopPropagation();b=$(this).attr("val");$(this).parent().remove();l(b,!1);f(b);a.func(b,p)}),t.focus(function(){m();a.write&&z.css("color","#ccc")}).blur(function(){a.write&&z.css("color","#888")}).keyup(function(b){I[b.keyCode]||(z[t.val()||w?"hide":"show"](),D[t.val()?"show":"hide"](),C!=t.val()&&(C=t.val(),a.funcKeyup(C),n.val(0),r=0))}),D.click(function(b){b.stopPropagation();f(0);t.val("");z.show();
a.func(0,p)}));n.o=a;n.value=f;n.process=function(){t.addClass("_busy")};n.cancel=function(){t.removeClass("_busy")};n.spisok=function(b){a.spisok=b;d();c();n.cancel()};n.title=function(){return A[n.val()]};n.inp=function(){return t.val()};n.focus=function(){t.focus()};return window[p+"_select"]=n}};
