...
7:"июля",8:"августа",9:"сентября",10:"октября",11:"ноября",12:"декабря"},FullData=function(a,c,b,d){},URL=APP_HTML+"/index.php?"+VALUES,AJAX_MAIN=APP_HTML+"/ajax/main.php?"+VALUES+"&ajax=1",hashLoc,hashSet=function(a){if(a||a.p){hashLoc=a.p;var c=!0;switch(a.p){case "client":"info"==a.d&&(hashLoc+="_"+a.id);break;case "zayav":"info"==a.d?hashLoc+="_"+a.id:"add"==a.d?hashLoc+="_add"+(_num(a.id)?"_"+a.id:""):a.d||(c=!1);break;case "zp":"info"==a.d?hashLoc+="_"+a.id:c=!1;break;default:a.d&&(hashLoc+=
"_"+a.d,a.d1&&(hashLoc+="_"+a.d1))}c&&VK.callMethod("setLocation",hashLoc)}},_scroll=function(a,c){var b=_cookie("p"),d=_cookie("d");if(!b)return 0;b=COOKIE_PREFIX+"scroll_"+b+(d||"");d=_cookie(b+"_unit");if("set"==a)_cookie(b,VK_SCROLL),c&&_cookie(b+"_unit",c);else if("page"==a)_cookie(b+"_page",c);else if("clear"==a)_cookie(b,0),_cookie(b+"_page",1);else return d&&$("#"+d).css("position","relative").append('<div id="unit-select-show"></div>').find("#unit-select-show").animate({opacity:0.9},200).animate({opacity:0},
900,function(){$("#unit-select-show").remove()}),_num(_cookie(b))},pinEnter=function(){var a={op:"pin_enter",pin:$.trim($("#pin").val())};a.pin&&2<a.pin.length&&($(".vkButton").addClass("busy"),$.post(AJAX_MAIN,a,function(a){a.success?location.href=URL+"&p="+_cookie("p")+"&d="+_cookie("d")+"&id="+_cookie("id"):($(".vkButton").removeClass("busy"),$("#pin").val("").focus(),$(".red").html(a.text))},"json"))},pinConfirm=function(a){function c(){var a={op:"pin_enter",pin:$.trim($("#tpin").val())};a.pin?
...
window.ATTACH||(ATTACH={});var e=$(this),f=_num(e.val());a=$.extend({zayav_id:0,func:function(){}},a);e.wrap('<div class="_attach">');e.parent().append('<a class="_attach-add">прикрепить файл</a>');var h=e.parent(),g=h.find("._attach-add");f&&h.addClass("ex");g.click(function(){function b(){var a=_cookie("_attached");0!=a&&(clearInterval(s),g.removeClass("_busy"),u.css("visibility","visible"),1==a&&(f=_cookie("_attached_id"),$.post(AJAX_MAIN,{op:"attach_get",id:f},function(a){a.success&&($("#name").val(a.name),
$("#size").html(a.size),l.addClass("uploaded"),d.butSubmit("Сохранить"))},"json")))}var d=_dialog({top:40,width:400,head:"Загрузка нового файла",content:'<div id="attach-add-tab"><div class="_info">Типы файлов, которые возможно загрузить: <b>xls</b>, <b>xlsx</b>, <b>doc</b>, <b>docx</b>, <b>pdf</b>, <b>rtf</b>.</div><h1><form method="post" action="'+AJAX_MAIN+'" enctype="multipart/form-data" target="attach-frame"><input type="hidden" name="op" value="attach_upload" /><input type="file" name="f1" id="file" /></form><button class="vk">Выбрать файл</button></h1><iframe name="attach-frame"></iframe><table id="tab"><tr><td class="label r">Название файла:<td><input type="text" id="name" /><tr><td class="label r">Размер:<td><b id="size">0</b> Кб</table></div>',
butSubmit:"",submit:function(){var b={op:"attach_save",id:f,zayav_id:a.zayav_id,name:$.trim($("#name").val())};b.name?(d.process(),$.post(AJAX_MAIN,b,function(b){d.abort();b.success&&(d.close(),h.addClass("ex"),e.val(f),ATTACH[f]=b.arr,c(),a.func(f))},"json")):d.err("Не указано название файла")}}),l=$("#attach-add-tab"),g=l.find(".vk"),q=l.find("form"),u=$("#file"),s;u.change(function(){u.css("visibility","hidden");g.addClass("_busy");_cookie("_attached",0);_cookie("_attached_id",0);s=setInterval(b,
500);q.submit()})});c(f)};
...
500);c.submit();b.addClass("busy")}).on("click","._image-spisok .del",function(a){a.stopPropagation();for(var c=$(this);"A"!=c[0].tagName;)c=c.parent();c.hasClass("deleting")||(c.addClass("deleting"),a={op:"image_del",id:c.attr("val")},$.post(AJAX_MAIN,a,function(a){a.success?(c.remove(),$("._image-add").removeClass("dn")):c.removeClass("deleting")},"json"))}).on("click","._iview",function(a){function c(){var a=n.length,b=m+1>=a?0:m+1,c=n[m];g.find(".head em").html(1<a?"Фотография "+(m+1)+" из "+
a:"Просмотр фотографии");g.find(".dtime").html("Добавлена "+c.dtime);g.find(".image img").attr("src",c.link).attr("width",c.x).attr("height",c.y).on("load",e);g.find(".hide").html('<img src="'+n[b].link+'">')}function b(){g.find(".image").on("click",function(){var a=n.length;1==a?d():(m++,m>=a&&(m=0),c())})}function d(){g.remove();FOTO_HEIGHT=0;_fbhs();VK.callMethod("scrollWindow",h)}function e(){FOTO_HEIGHT=g.height();_fbhs()}a.stopPropagation();$("#_image-view").remove();var f=$(this);a=f.attr("val");
var h=VK_SCROLL;"IMG"!=f[0].tagName&&(f=f.find("img"));f='<div id="_image-view"><div class="head"><em class="_busy">&nbsp;</em><a>Закрыть</a></div><table class="image"><tr><td><img src="'+f.attr("src").replace("-s.","-b.")+'"></table><div class="about"><div class="dtime"></div></div><div class="hide"></div></div>';FB.append(f);var g=$("#_image-view"),n=[],m=1;e();g.find(".head a").click(d);g.find("img:first").on("load",e);$.post(AJAX_MAIN,{op:"image_view",id:a},function(a){g.find("._busy").removeClass("_busy");
a.success&&(n=a.img,m=a.n,c(),b())},"json")}).on("click","._next",function(){var a=$(this),c=a.attr("val");c&&(c=c.split(":"),c[1]&&!a.hasClass("_busy")&&(a.addClass("_busy"),window[c[0]].page=c[1],_scroll("page",_num(c[1])),$.post(AJAX_MAIN,window[c[0]],function(b){b.success?(a.after(b.spisok).remove(),_nextCallback()):a.removeClass("_busy")},"json")))}).ready(function(){FB=$("#frameBody");FH=$("#frameHidden");_fbhs();window.frameHidden.onresize=_fbhs;sortable();imageSortable();VK.callMethod("scrollWindow",
