...
switch(@$_POST['op']) {
		...
		if($viewer_id) {
			...
			if($r = query_assoc($sql, GLOBAL_MYSQL_CONNECT)) {
				if($r['worker'] && $r['ws_id'] == WS_ID)
					jsonError('Ётот пользователь уже €вл€етс€ сотрудником ¬ашей организации');
				if($r['worker'])
					jsonError('Ётот пользователь уже €вл€етс€ сотрудником другой организации');
			}
	...
	case 'setup_worker_del':
		...
		$u = _viewer($viewer_id);
		if($u['viewer_admin'])
			jsonError('—отрудник €вл€етс€ руководителем');

		if(!$u['viewer_worker'])
			jsonError('ѕользователь уже не €вл€етс€ сотрудником');

		if($u['viewer_ws_id'] != WS_ID)
			jsonError('—отрудник не из этой организации');
...
//		GvaluesCreate();
	...
	case 'setup_invoice_edit':
		...
		//формирование списка сотрудников, которым доступен счЄт
		$old = array();
		if($r['visible'])
		...
		$changes =
			_historyChange('Ќаименование', $r['name'], $name).
			_historyChange('ќписание', _br($r['about']), _br($about)).
			_historyChange('ѕодтверждение поступлени€ на счЄт', _daNet($r['confirm_income']), _daNet($income)).
			_historyChange('ѕодтверждение перевода', _daNet($r['confirm_transfer']), _daNet($transfer)).
			_historyChange('¬идимость дл€ сотрудников', $r['visible'], $visible, $old, $new);
	...
	case 'expense_category_edit':
		...
		$changes =
			_historyChange('Ќаименование', $r['name'], $name).
			_historyChange('—писок сотрудников', $r['worker_use'] ? 'да' : 'нет', $worker_use ? 'да' : 'нет');
