		...
		if($send['arr'] = query_arr($sql, GLOBAL_MYSQL_CONNECT)) {
			$send['html'] = utf8(
				...
				'<table id="ts-tab" class="bs10 dn">'.
					'<tr><td class="label r"> оличество:*<td><input type="text" id="count" /><span id="max">(max: <b></b>)</span>'.
					'<tr><td class="label r">÷ена продажи (за ед.):*<td><input type="text" id="cena" class="money" value="'._cena($r['cost_sell']).'" /> руб.'.
