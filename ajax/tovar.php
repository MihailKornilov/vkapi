...
switch(@$_POST['op']) {
	...
	case 'tovar_edit':
		...
		if($changes =
			_historyChange(' атегори€', _tovarCategory($r['category_id']), _tovarCategory($category_id)).
			_historyChange('Ќаименование', _tovarName($r['name_id']), _tovarName($name_id)).
			_historyChange('ѕроизводитель', _tovarVendor($r['vendor_id']), _tovarVendor($vendor_id)).
			_historyChange('ѕодробно', $r['name'], $name).
			_historyChange('ќписание', $r['about'], $about).
			_historyChange('ѕрименение', _tovarPosition($r['set_position_id']), _tovarPosition($set_position_id)).
			_historyChange('≈диница изменени€', _tovarMeasure($r['measure_id']), _tovarMeasure($measure_id))
		)   _history(array(
				'type_id' => 106,
				'tovar_id' => $tovar_id,
	...
	case 'tovar_cost_set'://»зменение закупочной стоимости и продажи
		if(!$tovar_id = _num($_POST['tovar_id']))
			jsonError();
		...
		if($changes =
			...
			_historyChange('ѕродажа', _cena($r['sum_sell']), $sum_sell)
		)   _history(array(
				'type_id' => 106,
				'tovar_id' => $tovar_id,
				'v1' => '<table>'.$changes.'</table>'
		...
		if($send['arr'] = query_arr($sql, GLOBAL_MYSQL_CONNECT)) {
			$send['html'] = utf8(
				...
				'<table id="ts-tab" class="bs10 dn">'.
					...
					'<tr><td class="label r">÷ена продажи (за '._tovarMeasure($r['measure_id']).'):*<td><input type="text" id="cena" class="money" value="'._cena($r['sum_sell']).'" /> руб.'.
