...
switch(@$_POST['op']) {
	case 'cache_clear':
		...
		//обновление значени€ скриптов и стилей приложени€
		$sql = "UPDATE `_setup`
				SET `value`=`value`+1
				WHERE `app_id`=".APP_ID."
				  AND `key`='VERSION'";
	...
	case 'pagehelp_get':
		...
		$send['edit'] = (SA ? utf8('<a class="add">–едактировать</a>') : '');
		$send['txt'] = utf8($r['txt']);
		$send['dtime'] = utf8('<div class="pagehelp_show_dtime">»зменено '.FullDataTime($r['updated']).'</div>');
		jsonSuccess($send);
		break;
	case 'pagehelp_edit':
		if(!SA)
	...
	case 'zayav_expense_edit'://расходы по за€вке
...
//		echo $f['type'];
		switch ($f['type']) {
			...
			case 'image/tiff':
				...
				MagickWriteImage($image, $tmp); //—охран€ем результат
				ClearMagickWand($image); //”дал€ем и выгружаем полученное изображение из пам€ти
